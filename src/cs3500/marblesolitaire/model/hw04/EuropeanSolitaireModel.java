...
/**
 * This class houses all the methods of EuropeanSolitaireModel, which is a type of solitaire game.
 * European solitaire has an octagonal board with one initial empty space, where you can move.
...
public class EuropeanSolitaireModel extends AbstractMarbleSolitaireModel {
   ...
   */
  public EuropeanSolitaireModel(int armThickness, int sRow, int sCol) throws
  ...
  protected boolean isInvalidPos(int col, int row) {
    return (row < armThickness - 1 && col < armThickness - 1 - row)
        || (col > (((armThickness * 2) - 2) + row) && row < armThickness - 1)
        || (row > ((armThickness * 2) - 2) && col < row - (armThickness * 2) + 2)
        || col > (((armThickness - 1) / 2) * 10) - row && row > ((armThickness * 2) - 2);
  }
